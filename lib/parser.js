...
Parser.prototype.init = function(data) {
  data = data.split(' ');
  this.data.summary.videos = Number(data[0]);
  this.data.summary.endpoints = Number(data[1]);
  this.data.summary.videoRequests = Number(data[2]);
  this.data.summary.caches = data[3];
  this.data.cacheSize = Number(data[4]);
};
